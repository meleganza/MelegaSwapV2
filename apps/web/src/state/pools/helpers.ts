...
export const transformVault = (vaultKey: VaultKey, vault: SerializedCakeVault): DeserializedCakeVault => {
  const {
    ...
    userData: {
      ...
      dexTokenAtLastUserAction: cakeAtLastUserActionAsString,
      lastDepositedTime,
      lastUserActionTime,
    },
  } = vault
  ...
  const dexTokenAtLastUserAction = new BigNumber(cakeAtLastUserActionAsString)
  let userDataExtra
  let publicDataExtra
  if (vaultKey === VaultKey.CakeVault) {
    ...
    publicDataExtra = { totalLockedAmount, totalDexTokenInVault }
  } else {
    ...
    publicDataExtra = { totalDexTokenInVault: cakeAsBigNumber }
  }
