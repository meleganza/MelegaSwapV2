...
import { GELATO_NATIVE } from 'config/constants/common'
...
export function useLPToken(tokenAddress?: string): [string, string, string] | undefined | null {
  ...
  const { data, status } = useSWRImmutable(chainId && address && ['fetchLPTokenInfo', chainId, address], async () => {
    const calls = ['factory', 'token0', 'token1'].map((method) => {
      return { address: address.toString(), name: method }
    })

    return multicall(lpTokenABI, calls, chainId)
  })

  const factory = data?.[0]?.[0]
  ...
  return useMemo(() => {
    if (!chainId || !address) return undefined
    if (status !== FetchStatus.Fetched) return null
    if (token1 && token1_) {
