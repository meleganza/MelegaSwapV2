...
const AutoAprCell: React.FC<React.PropsWithChildren<AprCellProps>> = ({ pool }) => {
  ...
  const boostedYieldAmount = useMemo(() => {
    return isLock ? getFullDisplayBalance(userData?.dexTokenAtLastUserAction, 18, 5) : 0
  }, [isLock, userData?.dexTokenAtLastUserAction])
  ...
  if (pool.vaultKey === VaultKey.CakeVault && vaultPosition === VaultPosition.None) {
    return (
      <>
        <Pool.BaseCell role="cell" flex={['1 0 50px', '4.5', '1 0 120px', null, '2 0 100px']} mb={2}>
          <Pool.CellContent>
