...
const LockedStakingApy: React.FC<React.PropsWithChildren<LockedStakingApyProps>> = ({
  ...
  const earningTokenBalance = useMemo(() => {
    return getBalanceNumber(currentLockedAmountAsBigNumber.minus(userData?.dexTokenAtLastUserAction))
  }, [currentLockedAmountAsBigNumber, userData?.dexTokenAtLastUserAction])

  const boostedYieldAmount = useMemo(() => {
    return getFullDisplayBalance(userData?.dexTokenAtLastUserAction, 18, 5)
  }, [userData?.dexTokenAtLastUserAction])
