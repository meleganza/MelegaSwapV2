...
export default function useAvgLockDuration() {
  const { totalLockedAmount, totalShares, totalDexTokenInVault, pricePerFullShare } = useCakeVault()

  const avgLockDurationsInSeconds = useMemo(() => {
    const flexibleCakeAmount = totalDexTokenInVault.minus(totalLockedAmount)
  ...
  }, [totalDexTokenInVault, totalLockedAmount, pricePerFullShare, totalShares])
