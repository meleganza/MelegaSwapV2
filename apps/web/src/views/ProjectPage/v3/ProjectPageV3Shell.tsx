/**
 * Project Page V3 — premium conversion surface (DexScreener / CMC density + Melega brand).
...
const SwapAnchor = styled.div`
  ...
  /* Chrome-only density: keep swap start + action bar reachable without long scroll */
...
const TransparencyDetails = styled.details`
  border-radius: 12px;
  border: 1px solid ${pp.line};
  background: rgba(255, 255, 255, 0.02);
  padding: 0;
...
const TransparencyBody = styled.div`
...
function MiniSpark({ values }: { values: number[] }) {
  if (values.length < 2) {
    return (
      <SparkStub viewBox="0 0 100 28" aria-hidden data-testid="project-v3-spark-stub">
  ...
  const pts = values
    ...
    .join(' ')
  return (
    <SparkStub viewBox="0 0 100 28" aria-hidden data-testid="project-v3-spark">
      <polyline points={pts} fill="none" stroke={pp.gold} strokeWidth="1.6" strokeLinejoin="round" />
    </SparkStub>
  )
}

export type ProjectPageV3Props = {
...
export const ProjectPageV3Shell: React.FC<ProjectPageV3Props> = ({
  document,
  marketsDocument,
  ...
  const allocationCategories = tokenomicsDocument?.allocationCategories?.filter((a) => a.label) ?? []
  const tokenomicsFacts = tokenomicsDocument?.facts?.filter((f) => f.label && f.value) ?? []
  const hasTokenomics = allocationCategories.length > 0 || tokenomicsFacts.length > 0
  ...
  return (
    <Page
      id="project-page-v3"
      data-testid="project-page-v3"
      data-project-page="v3"
      data-project-slug={document.slug}
    >
      {/* HERO — 40% identity | 60% chart + swap */}
      <DenseBand data-testid="project-v3-hero" data-project-section="hero">
        <Hero>
          <div data-testid="project-v3-hero-left">
            ...
            <Row style={{ marginTop: 8, gap: 8, flexWrap: 'wrap' }}>
              {website?.url ? (
                <Btn $ghost href={website.url} target="_blank" rel="noreferrer" data-testid="project-v3-website">
                  Website
                </Btn>
              ) : null}
              {tgLink?.url ? (
                <Btn $ghost href={tgLink.url} target="_blank" rel="noreferrer" data-testid="project-v3-telegram">
                  Telegram
                </Btn>
              ) : null}
              {xLink?.url ? (
                <Btn $ghost href={xLink.url} target="_blank" rel="noreferrer" data-testid="project-v3-twitter">
                  Twitter
                </Btn>
              ) : null}
              {discordLink?.url ? (
                <Btn $ghost href={discordLink.url} target="_blank" rel="noreferrer" data-testid="project-v3-discord">
                  Discord
                </Btn>
              ) : null}
            ...
            {contract ? (
              <ContractRow data-testid="project-v3-contract">
                <span>{shortContract}</span>
                <CopyBtn type="button" onClick={onCopy} data-testid="project-v3-copy-contract">
                  {copied ? 'Copied' : 'Copy'}
                </CopyBtn>
                <Btn
                  $ghost
                  ...
                  target="_blank"
                  rel="noreferrer"
                  data-testid="project-v3-explorer"
                >
          ...
          <RightCol data-testid="project-v3-hero-right">
            <ProjectCharts
              slug={document.slug}
            ...
            />
            <SwapAnchor id="project-v3-swap" data-testid="project-v3-swap">
              <ProjectTradingEmbed
                slug={document.slug}
                marketsDocument={marketsDocument}
      ...
      <DenseBand data-testid="project-v3-market" data-project-section="market" style={{ padding: 0 }}>
        <MarketStrip>
          {(
            [
              ['Price', priceValue],
      ...
      {/* ACTION BAR — conversion CTAs immediately under strip */}
      <DenseBand data-testid="project-v3-actions" data-project-section="actions" style={{ paddingTop: 8, paddingBottom: 8 }}>
        <ActionBar>
          ...
          <Btn href={`/liquidity-studio?view=add&chain=${chainId}`} data-testid="project-v3-liquidity">
            Liquidity
          </Btn>
          <Btn href={`/farms?create=1&chain=${chainId}`} data-testid="project-v3-farm">
            Farm
          </Btn>
      ...
      {/* PROJECT ECONOMY */}
      <DenseBand data-testid="project-v3-economy" data-project-section="economy">
        <BandHead>
        ...
        <EconomyGrid>
          <EconomyCard data-testid="project-v3-economy-liquidity">
            <EconomyTitle>
              Liquidity
              <MelegaExploreChainBadge chainId={chainId} />
            </EconomyTitle>
            <EconomyMeta>
              ...
              <span>Update · {market.lastUpdate || '—'}</span>
            </EconomyMeta>
            <MiniSpark values={[]} />
          </EconomyCard>
          <EconomyCard data-testid="project-v3-economy-farm">
            <EconomyTitle>
              Farm
              <MelegaExploreChainBadge chainId={chainId} />
            </EconomyTitle>
            <EconomyMeta>
              ...
              <span>Volume · —</span>
              <span>Rewards · —</span>
              <span>Pair · {chainFarms[0]?.displayLabel || pairLabel}</span>
              <span>Farms · {chainFarms.length || '—'}</span>
            </EconomyMeta>
            <MiniSpark values={[]} />
          </EconomyCard>
          <EconomyCard data-testid="project-v3-economy-pool">
            <EconomyTitle>
              Pool
              <MelegaExploreChainBadge chainId={chainId} />
            </EconomyTitle>
            <EconomyMeta>
              ...
              <span>Volume · —</span>
              <span>Rewards · —</span>
              <span>Pair · {chainPools[0]?.displayLabel || '—'}</span>
      ...
      <DenseBand data-testid="project-v3-claim" data-project-section="claim">
        <ClaimCard>
          <div>
            <Muted style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.06em' }}>OWNER?</Muted>
            <ClaimTitle>Claim this Project</ClaimTitle>
            <ClaimBody>Verify ownership, then manage logo, description, links and socials.</ClaimBody>
            <ClaimList>
              <li>logo</li>
              <li>description</li>
              <li>links</li>
      ...
      {hasAbout ? (
        <DenseBand data-testid="project-v3-about" data-project-section="about">
          <BandHead>
            <BandTitle>About</BandTitle>
            <BandMeta>project</BandMeta>
          </BandHead>
          {description ? <Muted style={{ color: 'rgba(255,255,255,0.78)', marginBottom: 8 }}>{description}</Muted> : null}
          {categories.length ? (
          ...
          {roadmapDocument?.milestones?.length ? (
            <div data-testid="project-v3-roadmap" style={{ marginTop: 8 }}>
              <BandMeta style={{ marginBottom: 4 }}>Roadmap</BandMeta>
              {roadmapDocument.milestones.slice(0, 5).map((m) => (
                <Muted key={m.id || m.title} style={{ display: 'block' }}>
                  · {m.title || 'Milestone'}
                </Muted>
              ))}
            </div>
          ...
          {hasTokenomics ? (
            <div data-testid="project-v3-tokenomics" style={{ marginTop: 8 }}>
              <BandMeta style={{ marginBottom: 4 }}>Tokenomics</BandMeta>
              {allocationCategories.slice(0, 6).map((a) => (
                <Muted key={a.id || a.label} style={{ display: 'block' }}>
                  · {a.label}
                  {a.percent != null ? ` · ${a.percent}%` : ''}
                </Muted>
              ))}
              {!allocationCategories.length
                ? tokenomicsFacts.slice(0, 6).map((f) => (
                    <Muted key={f.id || f.label} style={{ display: 'block' }}>
                      · {f.label}
                      {f.value ? ` · ${f.value}` : ''}
                    </Muted>
                  ))
      ...
      {/* TRANSPARENCY — drawer only (Developer / Machine never rendered) */}
      <div data-testid="project-v3-transparency" data-project-section="transparency">
        <TransparencyDetails>
          <summary>
          ...
          <TransparencyBody>
            ...
            <Muted>Full developer and machine docs live in Melega Docs — not on this public page.</Muted>
          </TransparencyBody>
        </TransparencyDetails>
      </div>
...
export default ProjectPageV3Shell
