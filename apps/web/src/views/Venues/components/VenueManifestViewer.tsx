...
interface VenueManifestViewerProps {
  manifest: Record<string, unknown>
  slug: string
}

const VenueManifestViewer: React.FC<VenueManifestViewerProps> = ({ manifest, slug }) => {
  ...
  return (
    <Card>
      ...
      <Text fontSize="12px" color="textSubtle">
        {t('Venue manifest viewer disclaimer')}
      </Text>
      {expanded && <Pre>{JSON.stringify(manifest, null, 2)}</Pre>}
      <Text fontSize="12px" color="textDisabled">
        {t('Manifest path')}: /registry/venues/{slug}.json
      </Text>
    </Card>
  )
}

export default VenueManifestViewer
