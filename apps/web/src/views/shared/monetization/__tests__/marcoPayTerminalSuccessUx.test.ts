...
describe('MARCO Pay terminal success UX', () => {
  ...
  it('keeps single-flight protection while payment is submitted or confirming', () => {
    ...
    expect(checkout).toContain("order.state === 'PAYMENT_CONFIRMED'")
    expect(checkout).toContain("order.state === 'ACTIVATING'")
  ...
  it('does not show BOOST ACTIVATED until canonical fulfilment is ACTIVE', () => {
    expect(checkout).toMatch(
      /if \(\s*order\.state === 'ONCHAIN_PENDING' \|\|\s*order\.state === 'PAYMENT_CONFIRMED' \|\|\s*order\.state === 'ACTIVATING'\s*\) \{\s*setStatus\('submitted'\)\s*setWalletStage\('idle'\)\s*setQuoteSummary\('Payment confirmed · activating your service'\)\s*return/,
    )
  ...
  it('hides every payment CTA after ACTIVE/FULFILLED and Close cannot start another payment', () => {
    ...
    expect(checkout).toContain("status === 'confirmed' || status === 'submitted'")
  })
