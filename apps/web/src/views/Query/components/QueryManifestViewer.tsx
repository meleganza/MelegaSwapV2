...
interface QueryManifestViewerProps {
  manifest: Record<string, unknown>
}

const QueryManifestViewer: React.FC<QueryManifestViewerProps> = ({ manifest }) => {
  ...
  return (
    <Card>
      ...
      <Text fontSize="12px" color="textSubtle">
        {t('Query manifest viewer disclaimer')}
      </Text>
      {expanded && <Pre>{JSON.stringify(manifest, null, 2)}</Pre>}
      <Text fontSize="12px" color="textDisabled">
        {t('Manifest path')}: /registry/query/index.json
      </Text>
    </Card>
  )
}

export default QueryManifestViewer
