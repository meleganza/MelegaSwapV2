...
interface EventManifestViewerProps {
  manifest: Record<string, unknown>
  slug: string
}

const EventManifestViewer: React.FC<EventManifestViewerProps> = ({ manifest, slug }) => {
  ...
  return (
    <Card>
      ...
      <Text fontSize="12px" color="textSubtle">
        {t('Event manifest viewer disclaimer')}
      </Text>
      {expanded && <Pre>{JSON.stringify(manifest, null, 2)}</Pre>}
      <Text fontSize="12px" color="textDisabled">
        {t('Manifest path')}: /registry/events/{slug}.json
      </Text>
    </Card>
  )
}

export default EventManifestViewer
