...
interface AssetManifestViewerProps {
  manifest: Record<string, unknown>
  slug: string
}

const AssetManifestViewer: React.FC<AssetManifestViewerProps> = ({ manifest, slug }) => {
  ...
  return (
    <Card>
      ...
      <Text fontSize="12px" color="textSubtle">
        {t('Asset manifest viewer disclaimer')}
      </Text>
      {expanded && <Pre>{JSON.stringify(manifest, null, 2)}</Pre>}
      <Text fontSize="12px" color="textDisabled">
        {t('Manifest path')}: /registry/assets/{slug}.json
      </Text>
    </Card>
  )
}

export default AssetManifestViewer
