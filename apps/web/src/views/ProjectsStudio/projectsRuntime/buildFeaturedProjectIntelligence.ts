...
export function buildFeaturedProjectIntelligence(
  ...
  const riskLabel =
    project.riskTier === 'low' ? 'Low' : project.riskTier === 'medium' ? 'Medium' : project.riskTier === 'high' ? 'High' : healthSummaryLabel(health)
  ...
  return [
    ...
    {
      label: 'Verification',
      value: project.verificationStatus === 'observed' ? 'Observed' : project.trustBadges.includes('canonical') ? 'Canonical' : 'Pending',
