...
describe('PASSPORT_MODULE_001 Hero Identity', () => {
  ...
  it('does not mount Modules 004–009 implementations (002–003 may exist after certification)', () => {
    const studioFiles = readdirSync(ROOT)
    expect(studioFiles.some((f) => /PassportProjects|PassportLiquidity|PassportActivity|PassportSecurity/.test(f))).toBe(
      false,
    )
    const screen = readFileSync(path.join(WEB, 'src/views/Passport/PassportScreen.tsx'), 'utf8')
    expect(screen).not.toContain('PassportProjects')
