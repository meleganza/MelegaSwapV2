...
describe('PASSPORT_MODULE_002 Portfolio Overview', () => {
  ...
  it('does not implement later Passport modules', () => {
    const files = readdirSync(ROOT)
    expect(files.some((f) => /PassportAssets|PassportProjects|PassportLiquidity|PassportActivity|PassportSecurity/.test(f))).toBe(
      false,
    )
  })
