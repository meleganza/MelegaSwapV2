...
import { loadProjectGovernanceDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/governance/
 * PP011 — melega.project-governance.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectGovernanceDocument(slug, new Date().toISOString())
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.revision}"`
