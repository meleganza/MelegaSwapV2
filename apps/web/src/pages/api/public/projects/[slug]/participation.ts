...
import { loadProjectParticipationDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/participation/
 * PP006 — melega.project-participation.v1 (public opportunities only)
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectParticipationDocument(slug, new Date().toISOString())
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.participationRevision}"`
