...
import { loadProjectLiquidityBuildingDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/liquidity-building/
 * PP007 — melega.project-liquidity-building.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectLiquidityBuildingDocument(slug, new Date().toISOString())
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.liquidityBuildingRevision}"`
