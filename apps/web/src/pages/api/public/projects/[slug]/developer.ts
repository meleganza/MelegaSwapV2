...
import { loadProjectDeveloperDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/developer/
 * PP010 — melega.project-developer.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectDeveloperDocument(slug, new Date().toISOString())
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.developerRevision}"`
