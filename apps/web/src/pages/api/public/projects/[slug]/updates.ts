...
import { loadProjectUpdatesDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/updates/
 * PP008 — melega.project-updates.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectUpdatesDocument(slug, new Date().toISOString())
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.updatesRevision}"`
