...
import { loadProjectMarketsDocument, normalizeProjectSlugInput } from 'registry/projects/identity'

/**
 * GET /api/public/projects/{slug}/markets/
 * PP005 — melega.project-markets.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectMarketsDocument(slug, { generatedAt: new Date().toISOString() })
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.marketRevision}"`
