 ...
 * PP003 — melega.project-readiness.v1
 */
const handler: NextApiHandler = (req, res) => {
  ...
  const body = loadProjectReadinessDocument(slug, { generatedAt: new Date().toISOString() })
  if (!body) {
    res.setHeader('Cache-Control', 'public, max-age=60')
  ...
  const etag = `"${body.projectRevision}:${body.calculationRevision}:${body.readiness.score}"`
