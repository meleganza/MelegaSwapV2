...
export function assertCanonicalRouteAuthority(payload: unknown): CanonicalMmnRouteState {
  ...
  if (state.hub !== 'bnb' || typeof state.global_execution_enabled !== 'boolean') {
    throw new Error('Canonical MMN global execution gate is invalid.')
  }
  ...
  for (const expected of MARCO_WAVE1_DIRECT_ROUTES) {
    ...
    if (actual.execution_enabled && (!state.global_execution_enabled || !actual.publicly_active || actual.paused)) {
      throw new Error(`Canonical MMN route ${expected.from}->${expected.to} has an unsafe execution state.`)
    }
    if (actual.publicly_active && (!actual.certified || actual.paused)) {
      throw new Error(`Canonical MMN route ${expected.from}->${expected.to} has an unsafe public state.`)
    }
  }
  ...
  if (!solana || !solanaRoutes.length) throw new Error('Canonical Solana route state is missing.')
  if (solana.paused && !solanaRoutes.every((route) => route.paused && !route.execution_enabled)) {
    throw new Error('Canonical Solana route pause is not represented.')
  }
  if (!solana.paused && solanaRoutes.some((route) => route.paused)) {
    throw new Error('Canonical Solana network and route pause states disagree.')
  }
