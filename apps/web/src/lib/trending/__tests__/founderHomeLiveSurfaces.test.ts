...
describe('Founder Home live-surface contracts', () => {
  ...
  it('exposes every approved visibility family and MARCO PAY in the commercial funnel', () => {
    ...
    expect(modal).toContain('Open MARCO Passport')
    expect(modal).not.toContain('onPassportResolved')
    expect(modal).not.toContain('no funds moved')
    expect(marcoPay).toContain('openMarcoPayHandoffWindow')
    expect(marcoPay).toContain("'marco-pay:paymentCreated'")
    expect(marcoPay).not.toContain('widgets/marco.js')
    expect(marcoPay).not.toContain('passportResolved')
    expect(documentSource).not.toContain('https://marco.melega.ai/widgets/marco.js')
    expect(documentSource).not.toContain('widgets/marco-pay')
