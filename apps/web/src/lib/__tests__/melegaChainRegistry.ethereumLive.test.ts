/**
 * MELEGASWAP_V2_ETHEREUM_LIVE — final execution gates.
 */
...
import { CAKE_ETH } from '@pancakeswap/tokens'

const WEB = path.resolve(__dirname, '../../..')
const REPO = path.resolve(WEB, '../..')

describe('MELEGASWAP_V2_ETHEREUM_LIVE', () => {
  it('Ethereum is LIVE with canonical contracts + Founder MARCO', () => {
    ...
    expect(CAKE_ETH.address.toLowerCase()).toBe(MELEGA_ETH_MARCO.toLowerCase())
  })

  it('Router SSOT matches web + smart-router + registry + V2 adapter', () => {
  ...
  it('switcher includes Ethereum; Avalanche remains PREPARING after Arb LIVE', () => {
  ...
  it('token registry, logos, farms, pools inventory present', () => {
    ...
    expect(list.tokens.filter((t: { chainId: number }) => t.chainId === 1).length).toBeGreaterThanOrEqual(5)
