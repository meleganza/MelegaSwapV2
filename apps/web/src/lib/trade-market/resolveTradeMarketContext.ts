...
const MARCO_PUBLIC_SOURCES: TradeMarketSource[] = [
  {
    id: 'registry',
    label: 'Melega presence registry',
    href: '/registry/presence/marco-bnb-canonical.json',
    status: 'ready',
  },
  {
    id: 'dexscreener',
  ...
  },
  {
    id: 'dextools',
    label: 'DexTools',
    href: `https://www.dextools.io/app/en/bnb/pair-explorer/${MARCO_BSC_ADDRESS}`,
    status: 'link',
  },
  {
    id: 'coingecko',
...
export function resolveTradeMarketContext(input: {
  ...
  const subgraphReady = input.tokenData?.exists && (input.tokenData.volumeUSD > 0 || input.tokenData.liquidityUSD > 0)
  ...
  if (!subgraphReady && !input.hasPairPrices && !input.hasSwaps) {
    if (input.tokenData && !input.tokenData.exists) {
      ...
      missingReasonDetail = isMarco
        ? 'MARCO/BNB pair not indexed in subgraph — public catalog links available'
        : 'Pair not indexed in subgraph'
    } else {
      missingReason = 'subgraph_empty'
      missingReasonDetail = isMarco
        ? 'Subgraph empty — use DexScreener, DexTools, or BscScan links in machine payload'
