#!/usr/bin/env node
/**
 * R787 — 88-route responsive screenshot matrix with per-card unavailable detection.
 */
...
const OUT = path.join(__dirname, '../docs/runtime/r787-screenshots')
const BASE = process.env.R787_BASE || process.env.R786_BASE || 'https://melega.finance'
...
async function audit(page) {
  return page.evaluate(() => {
    ...
    const cardSelectors = '[data-pool-card],[data-farm-card],[data-collectible-card],[data-liquidity-card],[class*="Card"],article,section'
...
async function main() {
  ...
  await writeFile(path.join(OUT, 'r787-matrix-report.json'), JSON.stringify(report, null, 2))
