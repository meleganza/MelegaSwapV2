#!/usr/bin/env node
/**
 * R005-E farms final fit + R006-A pools final alignment screenshots.
 */
...
const OUT_FARMS = path.resolve(__dirname, '../../../docs/screenshots/r005e-farms-final-fit')
const OUT_POOLS = path.resolve(__dirname, '../../../docs/screenshots/r006a-pools-final-alignment')
...
async function main() {
  ...
  const report = {
    task: 'R005-E + R006-A',
    screenshots: {
      farms: FARMS_SHOTS.map((s) => s.name),
      pools: POOLS_SHOTS.map((s) => s.name),
    },
  }
