...
const getBaseThumbStyles = ({ isMax, disabled }: StyledInputProps) => `
  ...
  appearance: none;
  background: ${disabled ? "#555" : "#f4c430"};
  box-shadow: 0 0 0 3px #111, 0 4px 12px rgba(0, 0, 0, 0.42);
  border: 2px solid ${disabled ? "#777" : "#ffe36d"};
  border-radius: 50%;
  ...
  transition: 160ms transform, 160ms box-shadow;
  &:hover {
...
export const BarProgress = styled.div<DisabledProp>`
  background: linear-gradient(90deg, #b88700, #f4c430);
  border-radius: 999px;
  filter: ${({ disabled }) => (disabled ? "grayscale(100%)" : "none")};
  height: 6px;
  position: absolute;
