      ...
      return orderBy(
        ...
        (pool: DeserializedPool<T>) => {
          ...
          if (pool.vaultKey) {
            ...
            if (pool.stakingTokenPrice && vault?.totalDexTokenInVault?.isFinite()) {
              totalStaked =
                +formatUnits(EthersBigNumber.from(vault.totalDexTokenInVault.toString()), pool?.stakingToken?.decimals) *
