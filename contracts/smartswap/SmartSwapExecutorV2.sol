...
/// @title SmartSwapExecutorV2
/// @notice User-only exact-in executor. Fee authority is the on-chain allowedVenue mapping.
/// @dev Local candidate only. Not production-activated. Does not inherit V1 (no intentSigner).
contract SmartSwapExecutorV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant INTENT_VERSION = 2;
    ...
    constructor(address treasury_, address wrappedNative_, address owner_) Ownable(owner_) {
        if (treasury_ == address(0) || wrappedNative_ == address(0) || owner_ == address(0)) {
            revert ZeroAddress();
        }
        treasury = treasury_;
    ...
    function setRouter(address router, bytes32 venueId, bool allowed) external onlyOwner {
        ...
        if (allowed) {
            _structuralRouteCostBps(venueId);
            allowedVenue[router] = venueId;
        } else {
            allowedVenue[router] = bytes32(0);
        }
    ...
    function execute(ExecutionIntent calldata intent, address[] calldata path)
        external
        payable
        whenNotPaused
        nonReentrant
    ...
    {
        uint256 feeAmount = _validate(intent, path);
        ...
        if (intent.nativeIn) {
            ...
            if (intent.nativeOut) {
                userOutput = ISmartSwapV2Router(intent.router)
    ...
    function _validate(ExecutionIntent calldata intent, address[] calldata path)
        internal
        view
        returns (uint256 feeAmount)
    {
        ...
        if (intent.inputAmount == 0) revert InvalidAmount();
        bytes32 storedVenue = allowedVenue[intent.router];
        if (storedVenue == bytes32(0) || storedVenue != intent.venueId) revert WrongRouter();
        uint256 onChainCost = _structuralRouteCostBps(storedVenue);
        uint16 expectedFeeBps = authorizedFeeBps(onChainCost);
        feeAmount = (intent.inputAmount * expectedFeeBps) / 10_000;
        if (intent.structuralRouteCostBps != onChainCost) revert WrongFee();
        if (intent.feeBps == 0 || intent.feeBps > MAX_PROTOCOL_FEE_BPS) revert FeeBypass();
        if (intent.feeBps != expectedFeeBps) revert WrongFee();
        if (intent.feeAmount != feeAmount) revert WrongFee();
    ...
    function _structuralRouteCostBps(bytes32 venueId) internal pure returns (uint256) {
        if (venueId == keccak256("melega-dex")) return 25;
        if (venueId == keccak256("pancakeswap")) return 25;
        if (venueId == keccak256("uniswap")) return 30;
        revert UnknownVenue();
    }
}
